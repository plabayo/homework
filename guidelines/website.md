...
- **Internal linking** `[Recommended]` — link related exercises to each
  ...
  page is the hub.
- **Structured data (JSON-LD)** `[Recommended]` — at the site root,
...
- **Breadcrumbs** `[Recommended]` — for exercises grouped by topic, a
...
- **Content Security Policy** `[Recommended]` — strict policy is
  achievable since we have no third-party scripts. Suggested baseline:
